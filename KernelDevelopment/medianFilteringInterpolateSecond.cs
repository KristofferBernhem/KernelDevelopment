...
namespace KernelDevelopment
{
    class medianFilteringInterpolateSecond
    {
        ...
        public static void Execute()
        {
            ...
            Console.Out.WriteLine("mean:" + meanVector[0]);
            ...
            for (int idx = index; idx < n; idx += stride) // grid stride loop.
            {
                ...
                while (inpIdx < (idx + 1) * depth - (windowWidth) * nStep) // main loop over the middle part of the vector.
                {
                  ...
                  //  swapped = false;
                    float minima = 100;
                    int ind = 0;
                    while (j < low + 2 * windowWidth)// && Math.Abs(inputVector[inpIdx - (windowWidth) * nStep] - filterWindow[j]) > .001)
                    {                        
                        if (Math.Abs(inputVector[inpIdx - (windowWidth) * nStep] - filterWindow[j]) < minima)
                        {
                            minima = Math.Abs(inputVector[inpIdx - (windowWidth) * nStep] - filterWindow[j]);
                            ind = j;
                        }
                        j++;
                    }
          ...
          //          high++;
            ...
            //        filtIdx++; //update counter on position to insert next entry to.
                ...
                {                 
                    ...
                    float minima = 100;
                    int ind = 0;
                    while (j < low + high - 1)// && Math.Abs(inputVector[inpIdx - (windowWidth) * nStep] - filterWindow[j]) > .001)
                    {
                        if (Math.Abs(inputVector[inpIdx - (windowWidth) * nStep] - filterWindow[j]) < minima)
                        {
                            minima = Math.Abs(inputVector[inpIdx - (windowWidth) * nStep] - filterWindow[j]);
                    ...
                    j = ind;
                    while (j < low + 2 * windowWidth) // if this is not the last entry.
                    {
                        temp = filterWindow[j + 1];
                        filterWindow[j + 1] = filterWindow[j];
                        filterWindow[j] = temp;
